/**
 * @file Toast Component
 * @description Toast notification component with imperative API
 */

import { hideToastAtom, toastAtom } from '@/store/ui.atom'
import type { ToastType } from '@/types/components.types'
import { clsx } from 'clsx'
import { useAtomValue, useSetAtom } from 'jotai'
import { type ReactElement, useEffect } from 'react'
import { createPortal } from 'react-dom'

/**
 * Toast icons for different types
 */
const ToastIcons: Record<ToastType, ReactElement> = {
  success: (
    <svg
      className="w-5 h-5"
      fill="currentColor"
      viewBox="0 0 20 20"
      role="img"
      aria-label="Success icon"
    >
      <path
        fillRule="evenodd"
        d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
        clipRule="evenodd"
      />
    </svg>
  ),
  error: (
    <svg
      className="w-5 h-5"
      fill="currentColor"
      viewBox="0 0 20 20"
      role="img"
      aria-label="Error icon"
    >
      <path
        fillRule="evenodd"
        d="M10 18a8 8 0 100-16 8 8 0 000 16zM8.707 7.293a1 1 0 00-1.414 1.414L8.586 10l-1.293 1.293a1 1 0 101.414 1.414L10 11.414l1.293 1.293a1 1 0 001.414-1.414L11.414 10l1.293-1.293a1 1 0 00-1.414-1.414L10 8.586 8.707 7.293z"
        clipRule="evenodd"
      />
    </svg>
  ),
  warning: (
    <svg
      className="w-5 h-5"
      fill="currentColor"
      viewBox="0 0 20 20"
      role="img"
      aria-label="Warning icon"
    >
      <path
        fillRule="evenodd"
        d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z"
        clipRule="evenodd"
      />
    </svg>
  ),
  info: (
    <svg
      className="w-5 h-5"
      fill="currentColor"
      viewBox="0 0 20 20"
      role="img"
      aria-label="Info icon"
    >
      <path
        fillRule="evenodd"
        d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-7-4a1 1 0 11-2 0 1 1 0 012 0zM9 9a1 1 0 000 2v3a1 1 0 001 1h1a1 1 0 100-2v-3a1 1 0 00-1-1H9z"
        clipRule="evenodd"
      />
    </svg>
  ),
}

/**
 * Toast styles for different types
 */
const ToastStyles: Record<ToastType, string> = {
  success: 'bg-green-600 text-white',
  error: 'bg-red-600 text-white',
  warning: 'bg-yellow-600 text-white',
  info: 'bg-blue-600 text-white',
}

/**
 * Toast Container Component
 *
 * This component should be placed at the root of your app
 *
 * @example
 * // In App.tsx
 * <ToastContainer />
 */
export function ToastContainer() {
  const toast = useAtomValue(toastAtom)
  const hideToast = useSetAtom(hideToastAtom)

  // biome-ignore lint/correctness/useExhaustiveDependencies: hideToast is a stable setter and doesn't need to be in deps
  useEffect(() => {
    if (toast?.duration && toast.duration > 0) {
      const timer = setTimeout(() => {
        hideToast()
      }, toast.duration)

      return () => clearTimeout(timer)
    }
    return undefined
  }, [toast])

  if (!toast) {
    return null
  }

  const content = (
    <div className="fixed top-4 right-4 z-50 animate-in slide-in-from-top-5">
      <div
        className={clsx(
          'flex items-center gap-3 px-4 py-3 rounded-lg shadow-lg min-w-[300px] max-w-md',
          ToastStyles[toast.type]
        )}
        role="alert"
      >
        <div className="flex-shrink-0">{ToastIcons[toast.type]}</div>

        <p className="flex-1 text-sm font-medium">{toast.message}</p>

        <button
          type="button"
          onClick={hideToast}
          className="flex-shrink-0 opacity-70 hover:opacity-100 transition-opacity"
          aria-label="Close toast"
        >
          <svg
            className="w-4 h-4"
            fill="currentColor"
            viewBox="0 0 20 20"
            role="img"
            aria-label="Close"
          >
            <path
              fillRule="evenodd"
              d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
              clipRule="evenodd"
            />
          </svg>
        </button>
      </div>
    </div>
  )

  return createPortal(content, document.body)
}

/**
 * Imperative Toast API
 *
 * Use this with Jotai atoms:
 * - showSuccessToastAtom
 * - showErrorToastAtom
 * - showWarningToastAtom
 * - showInfoToastAtom
 *
 * @example
 * import { useSetAtom } from 'jotai'
 * import { showSuccessToastAtom } from '@/store/ui.atom'
 *
 * const showSuccess = useSetAtom(showSuccessToastAtom)
 * showSuccess('Operation completed successfully!')
 */
